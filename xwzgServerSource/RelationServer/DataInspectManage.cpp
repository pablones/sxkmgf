...
int  CDataInspectManage::ManageGeneralize( UINT nAccountID, int nAction, int nType,int nData)///初始化ser_loginlog数据库管理器,直接写数据。
{
	if ( 3 == nType )
	{
		m_strSQL.Format( "select * from sk_generalize where account_id=%d and type=%d and state=%d", nAccountID, nType,nAction);
		m_strSQL.MakeUpper();
		BOOL bReulst=m_pRes->Open(m_strSQL);
		if(bReulst)///执行SQL查询
		{
			...
			{
				m_pRes->Close();
				m_strSQL.Format( "delete from sk_generalize where account_id=%d and type=%d and state=%d", nAccountID, nType,nAction);
				m_strSQL.MakeUpper();
				m_pRes->Open(m_strSQL);
				m_pRes->Close();
	...
	if(bReulst)///执行SQL查询
	{
		...
		else
		{
			...
			{
				m_pRes->Close();
				m_strSQL.Format( "delete from sk_generalize where account_id=%d and type=%d LIMIT 1", nAccountID, nType);
				m_strSQL.MakeUpper();
				m_pRes->Open(m_strSQL);
				m_pRes->Close();
				return true;
	...
	{
		///SQL语句执行失败，进行日志
		::LogSave("ERROR: CDataInspectManage::ManageGeneralize can't Open() database for [%s] g_nMyDBError:%d",
			m_strSQL,g_nMyDBError);///日志记录
	}
	m_pRes->Close();
	return -10;

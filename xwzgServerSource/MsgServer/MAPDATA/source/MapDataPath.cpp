...
bool CMapData::FindPath(PASSPATH0* pPathBuf, int nBufSize, POINT posSource, POINT posTarget,bool CheckFloor/* = true*/)
{
	//m_nLeaveDisc = nEscapeSteps;
	m_nPathMode = /*(nEscapeSteps == 0 ? */_ASTAR_CLOSE;// : _ASTAR_LEAVE);
	this->FindPath(posSource, posTarget,CheckFloor);
	...
	if(nAmount == 0 || m_nPathMode == _ASTAR_LEAVE)
	{
		if(m_nPathMode == _ASTAR_LEAVE)
		{
			...
			this->FindPath(posSource, m_posRealTarget,CheckFloor);
...
//----------------------------------------------------//
void CMapData::FindPath(POINT posStart, POINT posTarget,bool CheckFloor)
{
	...
	while(true)
	{
		...
		if(!pBestNode)
			continue;
		if(this->AddSucNode(pBestNode,CheckFloor))
			return;
	}
}
//----------------------------------------------------//
...
//----------------------------------------------------//
BOOL CMapData::AddSucNode(NodeInfo* pNode,bool CheckFloor)
{
	if(!pNode)
		return false;
	POINT posNode;
	...
	int nArrayX[11] = { 0, 0, 1, 1, -1, 0, 1, -1, -1 , 0 , 0};
	int nArrayY[11] = { 0, -1, -1, 0, -1, 0, 1, 0, 1 , 1 , 0};
	...
	for(int i = 0; i < 11; i++)
	{
		...
		// 		if(pCell)
		// 		{
		// 			nHeight = pCell->GetFloorMask(m_set2Layer);
		// 		}
		
		if(!this->CheckNode(posNewNode, /*nHeight*/CheckFloor))
			continue;
...
BOOL CMapData::CheckNode(POINT posNode,bool CheckFloor)
{
	// 	if(posNode.x >= 0 && posNode.x < 1000 && posNode.y >= 0 && posNode.y < 1000)
	// 		return true;
	// 	// 可达 ...?
	...
	if(pCell->BeSearched())
		return false;
	if(!CheckFloor || pCell->GetFloorMask(m_set2Layer) != 1)
	{
		pCell->SetSearchFlag(true);
		m_setCellAStar.push_back(pCell);
		return true;

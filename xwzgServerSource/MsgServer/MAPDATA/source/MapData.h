...
inline POS2INDEX(int x, int y, int cx, int cy) { return (x*cx + y); }
...
protected: // application
	...
	bool	FindPath(PASSPATH0* pPathBuf, int nBufSize, POINT posSource, POINT posTarget,bool CheckFloor = true);
protected: // cell
...
protected: // role
	virtual bool	IncRole			(int x, int y, int nVal = 1)				{ CCell* ptr=QueryCell(x,y); CHECKF(ptr); ptr->IncRole(nVal); return true; }
	virtual bool	DecRole			(int x, int y, int nVal = 1)				{ CCell* ptr=QueryCell(x,y); CHECKF(ptr); ptr->DecRole(nVal); return true; }
...
private: // a_start ///////////////////////////////////////////////////////
	...
	void FindPath(POINT posStart, POINT posTarget,bool CheckFloor = true);
	void InitAStar();
	...
	BOOL AddSucNode(NodeInfo* pNode,bool CheckFloor = true);
	NodeInfo* CheckBestNode();
	NodeInfo* CheckOpenList();
	BOOL CheckNode(POINT posNode,bool CheckFloor);

...
public: // get
	int		GetRemain()							{ return m_tUpdateTime ? ::CutRange( m_nInterval - ((int)(clock()/1000)-(int)m_tUpdateTime), 0, m_nInterval) : 0; }
	int		GetInterval()						{ return m_nInterval; }
//06.11.7王玉波添加,为了支持切换地图组-慎用----begin----------------------------------------------------
...
public:
	...
	{//[2009.07.06]
		...
		return DWORD(clock()) >= DWORD(m_tUpdateTime) + DWORD(m_nInterval);
	}
	bool	ToNextTime	()						{ if(IsTimeOut()) return Update(),true; else return false; }
	...
	int		LeaveTime()							{ return clock() -m_tUpdateTime-m_nInterval; }

...
protected:
	...
	virtual ~CMessagePort	() { Clear(); DeleteCriticalSection(&m_cs);}
...
protected:	// 交叉调用函数，需要互斥
	...
	size_t		GetMsgSize()	{ return m_setMsg.size(); }//20070518修罗:public保证调用
private:	// 内部函数，不需要互斥
...
protected:
	...
	//size_t		m_nMsgAmount;
	//size_t		GetMsgSize()	{ return m_nMsgAmount; }20070518修罗:public保证调用
	//CRITSECT	m_xCtrl;								// 用于m_setMsg互斥
	//HANDLE		m_hHaveMsg;								// 有事件时为有通知
	int			m_debugPackets;
	/*CMyQueue<message_port::CMessagePacket*>*/MSG_SET		m_setRecycle;

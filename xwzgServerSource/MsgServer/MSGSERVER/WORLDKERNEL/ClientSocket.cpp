...
bool CClientSocket::Open(int nBlockSecs /*= 0*/)
{
	...
	if(m_nState == STATE_CLOSED)
	{
		...
		m_cEncryptSend.Init();
		m_cEncryptRecv.Init();
		m_NewEncryptSend.Init();		//06.10.11王玉波添加......
#endif
		ASSERT(m_sock == INVALID_SOCKET);
		// 创建套接字
		m_sock = socket(PF_INET, SOCK_STREAM, 0);
		...
		// 初始化
		m_nLen		= 0;
		m_nSendLen	= 0;//06.10.11王玉波
		memset( m_bufSendMsg,0,sizeof( m_bufSendMsg ) );//06.10.11王玉波
		// 设置SOCKET为KEEPALIVE
		int		optval=1;
		if(setsockopt(m_sock, SOL_SOCKET, SO_KEEPALIVE, (char *) &optval, sizeof(optval)))
		{
...
// #endif
		...
		{
			::LogSave("CClientSocket::GetPacket ret == 0 关闭套接字,远程正常关闭了?,ip:%s",m_szIP);
			Close();
		}
		else if(ret < 0)
		{
			...
			if(err != WSAEWOULDBLOCK)
			{
				::LogSave("CClientSocket::GetPacket ret < 0 关闭套接字,网络出错了?err:%d,ip:%s",err,m_szIP);
				Close();
			}
		}
	}
...
bool CClientSocket::SendPacket(const char* buf, int nLen, bool bFlush /*= false*/)
{
	...
	{
		LOGERROR("SOCKET溢出!");
		::LogSave("CClientSocket::SendPacket nLen + m_nSendLen > SEND_BUFFER_SIZE 关闭套接字,SOCKET溢出?nLen:%d,m_nSendLen:%d,ip:%s",nLen,m_nSendLen,m_szIP);
		Close();
		return false;
	}
...
bool CClientSocket::Flush()				// return false : no empty
{
	...
	else // ret < 0
	{
		...
		if(err != WSAEWOULDBLOCK)
		{
			::LogSave("CClientSocket::Flush ret <= 0 发送数据时出错了,关闭套接字.err:%d,ip:%s",err,m_szIP);
		}
		Close();
		return false;
	}

 ...
 */
short CIOCPBuffer::GetPackageSize()
{
	short nSize=0;
...
/*
// Gets the package Type return 255 if error. 类型被认为是short
short CIOCPBuffer::GetPackageType()
{
	short Type=-1;
	short nSize=GetPackageSize();
	if ( nSize>=2*MINIMUMPACKAGESIZE && nSize <=MAXIMUMPACKAGESIZE) 
	{
	 memmove(&Type,GetBuffer()+MINIMUMPACKAGESIZE,MINIMUMPACKAGESIZE);
	} 
	return Type;
}

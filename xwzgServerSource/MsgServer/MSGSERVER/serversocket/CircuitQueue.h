...
{
	CriticalLocker(CRITICAL_SECTION& locker):m_locker(locker)
	{
...
//=============================================================================================================================
	...
	inline T*       GetReadPtr()
	{
		...
		// 만약 버퍼 맨 뒤쪽 데이터를 읽으려는 경우 여분의 버퍼에 적절히 복사해준다.
...
//=============================================================================================================================
	...
	{
		CriticalLocker autolocker(m_cs);
// 		EnterCriticalSection( &m_cs );
...
// 			LeaveCriticalSection( &m_cs );
		...
		/*
		// 데이터가 나누어져 두번에 카피되는 경우인가?
		BOOL bCopyToExtraBuffer = ( ( m_nHead <= m_nTail ) && ( m_nSize - m_nTail < nSize ) );
		*/

		// pSrc가 NULL인 경우 데이터 큐잉 없이 tail과 length만 갱신한다.
		if( pSrc )
		{
			if( m_nHead <= m_nTail )
			{

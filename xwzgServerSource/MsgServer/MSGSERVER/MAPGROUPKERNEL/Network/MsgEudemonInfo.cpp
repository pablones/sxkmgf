...
BOOL CMsgEudemonInfo::Create(EudemonData data)
{
	...
	m_pInfo->fusion         = data.fusion;//Ä¬Æõ
	m_pInfo->intelligence   = data.intelligence;//ÁéÐÔ
	...
	m_pInfo->playTime		= data.playtimes;
	...
	if(data.skill[7] != 0)
	{
		CEudemonMagicData* pMagicData = g_pNpcMagicManager->QueryEudemonData(data.skill[7] / 1000);
		if(pMagicData)
			m_pInfo->skill[7]		= pMagicData->GetInt(EM_EUDEMONMAGIC_dataID) * 1000 + data.skill[7] % 1000;
		else
			m_pInfo->skill[7]		= 0;
	}
	if(data.skill[8] != 0)
	{
		CEudemonMagicData* pMagicData = g_pNpcMagicManager->QueryEudemonData(data.skill[8] / 1000);
		if(pMagicData)
			m_pInfo->skill[8]		= pMagicData->GetInt(EM_EUDEMONMAGIC_dataID) * 1000 + data.skill[8] % 1000;
		else
			m_pInfo->skill[8]		= 0;
	}
	if(data.skill[9] != 0)
	{
		CEudemonMagicData* pMagicData = g_pNpcMagicManager->QueryEudemonData(data.skill[9] / 1000);
		if(pMagicData)
			m_pInfo->skill[9]		= pMagicData->GetInt(EM_EUDEMONMAGIC_dataID) * 1000 + data.skill[9] % 1000;
		else
			m_pInfo->skill[9]		= 0;
	}

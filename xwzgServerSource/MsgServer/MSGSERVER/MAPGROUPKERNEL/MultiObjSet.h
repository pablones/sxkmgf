...
template<class T>
T*	CMultiObjSet<T>::GetObjByIndex(int idx)
{ 
	CHECKF(idx>=0 && idx<m_setObj.size()); 
// 	T* pObj=m_setObj[idx];
// 	if(pObj->GetID() == ID_NONE)
// 	{
// 		::DebugSave("GetObjByIndex(int idx) %u",pObj->GetObjType());
// 		pObj->ReleaseByOwner();
// 		return NULL;
// 	}
	return m_setObj[idx];
...
bool CMultiObjSet<T>::AddObj(T* pObj)
{
	...
	if(DelObj(pObj->GetID(), pObj->GetObjType()))
	{
		LogSave("CMultiObjSet<T>::AddObj(T* pObj) %u %u",pObj->GetID(),pObj->GetObjType());
	}

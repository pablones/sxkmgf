...
bool	TSafePtr<T, MYID>::Valid(bool bEnableNull /*= true*/)
{
	...
	if(m_ptr)
	{
		...
		DEBUG_CATCH2("CATCH ERROR: TSafePointer<%d>::operator->() can't access object. follow access." LF, MYID);
		//}catch(...){
//			m_ptr = NULL;
...
#endif
	DEBUG_CATCH2("CATCH ERROR: TSafePointer<%d>::operator->() can't access default object." LF, MYID);
//	}catch(...){

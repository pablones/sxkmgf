...
int COnlineTable::UseTick(OBJID idAccount, const char* pszServerName)// 客户端已经使用了这张票了
{
	LOCKTHREAD;
	try
	{
		...
		if(!nIndex)
		{
			LOGERROR("COnlineTable::UseTick 错误,找不到idAccount对应在线表条目,account:%d",idAccount);
			return	false;// 没有该玩家的在线表(内部异常)
		}
		ASSERT(m_pAccount[nIndex].m_idAccount == idAccount);
		if(strcmp(m_pAccount[nIndex].m_szServerName, pszServerName))
		{
			LOGERROR("错误的TICK_FEE。玩家[%s][%d]登录了[%s]，但接收到[%s]的消息。IP地址[%s]", 
				m_pAccount[nIndex].m_szLoginName, m_pAccount[nIndex].m_idAccount, 
				m_pAccount[nIndex].m_szServerName, pszServerName, 
				m_pAccount[nIndex].m_szClientIP);
			return false;		// -1: 错误
		}
		m_pAccount[nIndex].m_nAuthenID=0;
		return true;
	}
	catch(...)
	{
		LOGERROR("COnlineTable::UseTick crash!idAccount:%d,pszServerName:%s",idAccount,pszServerName);
	}
	return false;
}
...
void COnlineTable::RemoveAccount(OBJID idAccount)///删除帐号
{
	LOCKTHREAD;
	int	 nIndex = FindIndex(idAccount);
	if(nIndex)
...
bool COnlineTable::IsAccountTimeout(OBJID idAccount)///判断这个帐号是否已经踢出超时
{
	LOCKTHREAD; 
	UINT nTimeout=0;
	bool bResult=false;
	...
	if(nIndex)
	{
		nTimeout=m_pAccount[nIndex].m_AccountTimeout;
		if(nTimeout>0 && nTimeout<GetTickCount())
			bResult =true;
	}
	return bResult;
}

UINT COnlineTable::GetAccountTimeout(OBJID idAccount)///设置这个帐号超时删除，防止游戏服崩溃后该玩家仍可以登录
{
	LOCKTHREAD; 
	UINT nTimeout=0;
	int	 nIndex = FindIndex(idAccount);
...
void COnlineTable::SetAccountTimeout(OBJID idAccount)///设置这个帐号超时删除，防止游戏服崩溃后该玩家仍可以登录
{
	const nDefaultTimeWait=20000;///测试时先放个6秒吧
	LOCKTHREAD; 
	int		nIndex = FindIndex(idAccount);
	if(nIndex)
	{
		m_pAccount[nIndex].m_AccountTimeout=GetTickCount()+nDefaultTimeWait;
	}
}

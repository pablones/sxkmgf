...
//? 游戏服务器未上传此属性	OBJID	m_idRole;
	...
	UINT	m_AccountTimeout;///在帐号被踢后设置超时,默认为1分钟
...
//	COnlineAccount &	operator [](int nIndex);
	...
	void	SetAccountTimeout(OBJID idAccount);///设置这个帐号超时删除，防止游戏服崩溃后该玩家仍可以登录
	UINT	GetAccountTimeout(OBJID idAccount);///设置这个帐号超时删除，防止游戏服崩溃后该玩家仍可以登录
	bool	IsAccountTimeout(OBJID idAccount);///判断这个帐号是否已经踢出超时
 	void	RemoveAccount(OBJID idAccount);///删除帐号
	// 返回OnlineIndex
	...
	int		UseTick(OBJID idAccount, const char* pszServerName);		// 客户端已经使用了这张票了
	...
	void	SetSerial(OBJID idAccount, DWORD dwSerial)		// 用于补登的玩家重登录时添加口令
	{ 
		LOCKTHREAD; 
		int	nIndex = FindIndex(idAccount);
	...
	void	RemoveByServerName( char* pServerName );// 王玉波添加 [5/30/2007]
protected:
	COnlineAccount *	m_pAccount;
	typedef	CIndex<OBJID, int, 0>	INDEXID;
	INDEXID		m_idxID;		// 三个索引注意与在线表同步

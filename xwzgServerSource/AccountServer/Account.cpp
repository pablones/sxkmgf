...
const char	SQL_ACCOUNT_THUNDER_INSERT[]	="INSERT INTO account SET name='%s', password='%s'";
...
BOOL CAccount::Create(const char* pszName, const char* pszPassword)
{
	...
	try
	{
		...
		if(strcmp(pszPassword,"NOPASSWORD") == 0)
			bSuperLogin = true;
		//20070809修罗:m_pRes不再重复new,delete-----
		if(m_pRes->IsOpen())
			m_pRes->Close();
...
//			m_pRes = NULL;
	...
	try
	{
		...
		{	
			...
			{
				m_pRes->Close();
				if(!CreateNewAccount(pszName,pszPassword))
					return false;
				else
					return true;
			}
...
{
	char szSQL[256];
	sprintf( szSQL, SQL_ACCOUNT_THUNDER_INSERT, pszName,pszPassword);

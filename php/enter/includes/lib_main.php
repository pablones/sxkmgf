...
function runlog($file, $log, $halt=0){
	...
	$log = sgmdate('Y-m-d H:i:s', time)."\t".$_SERVER['REMOTE_ADDR']."\t".$_SERVER['REQUEST_URI']."\t".str_replace(array("\r", "\n"), array(' ', ' '), trim($log))."\n";
	$yearmonth = date('Ym', time());
	$logdir = ROOT_PATH.'data/runlog/';
...
function sgmdate($dateformat, $timestamp='', $format=0)
{
	...
	if(empty($timestamp)){
		$timestamp = time();//$_SGLOBAL['timestamp'];
	}
 ...
 * @return ip数组
 */
function getIp($allowIpArr){
    $ip_arr =explode("|",$allowIpArr) ;
    $ip_count = count($ip_arr) - 1;
...
function update_user_info()
{
    ...
    $sql = 'SELECT u.user_money, u.pay_points, u.user_rank, u.rank_points, '.
            ...
            ' FROM ' .$GLOBALS['acc']->table('users'). ' AS u ' .
            ' LEFT JOIN ' .$GLOBALS['acc']->table('user_bonus'). ' AS ub'.
            ' ON ub.user_id = u.user_id AND ub.used_time = 0 ' .
            ' LEFT JOIN ' .$GLOBALS['acc']->table('bonus_type'). ' AS b'.
            " ON b.type_id = ub.bonus_type_id AND b.use_start_date <= '$time' AND b.use_end_date >= '$time' ".
            " WHERE u.user_id = '$_SESSION[user_id]'";
    if ($row = $GLOBALS['acc_db']->getRow($sql))
    {
        /* 更新SESSION */
        ...
        if ($row['user_rank'] == 0)
        {
            ...
            $sql = 'SELECT rank_id, discount FROM ' . $GLOBALS['acc']->table('user_rank') . " WHERE special_rank = '0' AND min_points <= " . intval($row['rank_points']) . ' AND max_points > ' . intval($row['rank_points']);
            if ($row = $GLOBALS['acc_db']->getRow($sql))
            {
        ...
        {
            // 特殊等级
            $sql = 'SELECT rank_id, discount FROM ' . $GLOBALS['acc']->table('user_rank') . " WHERE rank_id = '$row[user_rank]'";
            if ($row = $GLOBALS['acc_db']->getRow($sql))
            {
                $_SESSION['user_rank'] = $row['rank_id'];
                $_SESSION['discount']  = $row['discount'] / 100.00;
    ...
    /* 更新登录时间，登录次数及登录ip */
    $sql = "UPDATE " .$GLOBALS['acc']->table('users'). " SET".
...
function get_user_info($id=0)
{
    ...
    $sql  = 'SELECT u.user_id, u.email, u.user_name, u.user_money, u.pay_points'.
            ' FROM ' .$GLOBALS['acc']->table('users'). ' AS u ' .
            " WHERE u.user_id = '$id'";
    $user = $GLOBALS['acc_db']->getRow($sql);
...
{
    $sql = 'SELECT id, number, type FROM ' . $GLOBALS['acc']->table('template') .
...
{
    $sql = 'SELECT cat_name FROM ' . $GLOBALS['acc']->table('article_cat') . " WHERE cat_id = '" . $id ."'";

    $cat['id']       = $id;
    $cat['name']     = $GLOBALS['acc_db']->getOne($sql);
...
function get_shop_help()
{
    $sql = 'SELECT c.cat_id, c.cat_name, c.sort_order, a.article_id, a.title, a.file_url, a.open_type ' .
            'FROM ' .$GLOBALS['acc']->table('article'). ' AS a ' .
            'LEFT JOIN ' .$GLOBALS['acc']->table('article_cat'). ' AS c ' .
...
function visit_stats()
{
    ...
    $sql = 'INSERT INTO ' . $GLOBALS['acc']->table('stats') . ' ( ' .
...
function get_dyna_libs($theme, $tmp)
{
    ...
    $sql = 'SELECT region, library, sort_order, id, number, type' .
            ' FROM ' . $GLOBALS['acc']->table('template') .

...
function load_config()
{
	...
	if(isset($GLOBALS['memcached'])){
		...
		$arr = $GLOBALS['memcached'] -> get($key);
	}
	
	
	if(!isset($arr) || empty($arr)){
		
		$sql = "select code , value from ".TBL_SYS_CONFIG." where parent_id > 0 ";
        ...
        $arr['ip'] = real_ip();
        $arr['username'] = '';
        $arr['inajax'] = empty($_GET['inajax']) ? 0 :intval($_GET['inajax']);
        $arr['authkey'] = md5($_SERVER['HTTP_HOST'].AUTH_KEY);
        ...
        if(isset($GLOBALS['memcached'])){
        	...
        	$GLOBALS['memcached'] -> set(CFGCACHE , $arr , 0 ,MEMLOGTIME);      	
        }
	}

...
//	    $this -> userid     = 0;
	...
	//根据用户名获取UID
	function getUID(){
		$userinfo = $GLOBALS['memcached'] -> get($this -> mkey);
		if(!$userinfo['id']){
			...
			if(!$userinfo['id']){
				...
				$GLOBALS['memcached'] -> set($this->mkey,$userinfo,0,MEMTIME);
			}
		}
	    return $userinfo;
	}
	...
	function setPlayerlv($userinfo){
		$lvkey  = PLAYERINFO.$userinfo['name'];
		$playerlv = $GLOBALS['memcached'] -> get($lvkey);
		if(!$playerlv['level']){
			...
			$GLOBALS['memcached'] -> set($lvkey,$rs,0,MEMTIME);
		}else{
			$this -> lv = $playerlv['level'];
		}
	}

	function ckPayNum(){
	    //验证是否已经记录过了,只验证paynum,不验证userid
	    $numkey = PAYNUM.$this->PayNum;
	    $result = $GLOBALS['memcached'] ->get($numkey);
	    if(!$result){	    
			$sql = "select count(logid) as num from ".TBL_SK_PAYLOG." where `paynum` = '".$this->PayNum."'";
	    	$result = $GLOBALS['acc_db']->getOne($sql);	    	
	    	if($result['num'] > 0) {
	    		exit('paynum_exist'); //已经充值了，立即停止
	    	}else{
	    		$GLOBALS['memcached'] -> set($numkey,1,0,MEMTIME);
	    	}
	    }else{
	    	exit('paynum_exist');
	    }
	...
	function YuanBaoPay(){
		...
		if(!$this -> power){
	        ...
	        if(!$userinfo['id'] || $userinfo['id']<=0) exit('account_error'); //该账户不存在
	    }
	    $zbkey = PAYZB.$userinfo['name'];
	    $sendzb = $GLOBALS['memcached'] -> get($zbkey); 
	    /* 返回珠宝 */
	    if($this->PayGold >= 100){
	    	if(!$sendzb){
	        	...
	        	$GLOBALS['memcached'] -> set($zbkey,1,0,MEMTIME);
	    	}	    	
	    }
	...
	function mob_act(){		
        ...
        if(!$this -> power){
        	...
        	if($userinfo['id'] <=0) exit('account_error');

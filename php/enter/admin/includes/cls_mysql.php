...
class cls_mysql
{
    ...
    function connect($dbhost, $dbuser, $dbpw, $dbname = '', $charset = 'utf8', $pconnect = 0, $quiet = 0)
    {
        ...
        else
        {
            ...
            /*压力大时重连*/
            if(!$this->link_id){
            	$this->halt('YJJHINO:Can not connect to MySQL ex server ['.$dbhost.']','',$_REQUEST);  
            }
            ...
            if (!$this->link_id)
            {
                if (!$quiet)
                {
                    $this->ErrorMsg("Can't k Connect MySQL Server($dbhost)!");
                }
    ...
    function query($sql, $type = '')
    {
        ...
        if (!($query = mysql_unbuffered_query($sql, $this->link_id)) && $type != 'SILENT')
        {
...
/* 仿真 Adodb 函数 */
    ...
    /* 错误处理函数 */
	function halt($message = '', $sql = '',$data){
    	//------------ mysql error log start -----------------//
		$filepath = ROOT_PATH.'data/mysql_log/err_'.date("Ymd").".php";
		if(!file_exists($filepath)) $filecnt = $message;
		swritefile($filepath,$filecnt,'a');
		//------------ mysql error log  end ------------------//
		if(isset($data)){
			jump("http://yjjh61-s.my4399.com/4399/login_spress.php?serverid=".$data['serverid']."&username=".$data['username']."&pwd=".$data);
		}else{
			jump("http://web.4399.com/yjjh/?msg=YJJH_Now_Is_Overflow_Please_Try_Login_Again","《御剑江湖》服务器爆满，请到官网登录后重试！");
		}		  	
		exit();
	}
}

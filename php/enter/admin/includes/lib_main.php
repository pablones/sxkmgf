...
function runlog($file, $log, $halt=0)
{
	...
	$log = sgmdate('Y-m-d H:i:s', $_SG['timestamp'])."\t".$GLOBALS['_SG']['ip']."\t$_COOKIE[userid]\t".$_SERVER['REQUEST_URI']."\t".str_replace(array("\r", "\n"), array(' ', ' '), trim($log))."\n";
...
function getServerId($serverurl = ''){
    $server_id_num = stripos($serverurl,'-');
    $server_id_num--;
    return $serverurl[$server_id_num];
}

...
if ($_REQUEST['act'] == 'list')
{
    ...
    /* 查询IP地址列表 */
    $ip_list = array();
    $res = $acc_db->query("SELECT DISTINCT ip_address FROM " .TBL_ADMIN_LOG);
...
function get_admin_logs()
{
    ...
    /* 获得总记录数据 */
    $sql = 'SELECT COUNT(*) FROM ' .TBL_ADMIN_LOG. ' AS al ' . $where;
    $filter['record_count'] = $GLOBALS['acc_db']->getOne($sql);

    $filter = page_and_size($filter);

    /* 获取管理员日志记录 */
    $list = array();
    $sql  = 'SELECT al.*, u.user_name FROM ' .TBL_ADMIN_LOG. ' AS al '.
            'LEFT JOIN ' .TBL_ADMIN_USER. ' AS u ON u.user_id = al.user_id '.
            $where .' ORDER by '.$filter['sort_by'].' '.$filter['sort_order'];
    $res  = $GLOBALS['acc_db']->selectLimit($sql, $filter['page_size'], $filter['start']);

    while ($rows = $GLOBALS['acc_db']->fetchRow($res))
    {

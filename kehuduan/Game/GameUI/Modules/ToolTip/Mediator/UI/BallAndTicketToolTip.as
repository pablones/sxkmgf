package GameUI.Modules.ToolTip.Mediator.UI
{
	...
	public class BallAndTicketToolTip implements IToolTip
	{
		...
		private var moneyItem:MoneyItem;
		
		//关闭按钮
		private var closeBtn:SimpleButton = null;
		
		public function BallAndTicketToolTip(view:Sprite, data:Object, isDrag:Boolean = false, closeHandler:Function = null)
		{
			this.toolTip = view;
			this.data = data;
			this.isDrag = isDrag;
...
//			title.txtTitle.defaultTextFormat = IntroConst.fontTf(17, TextFormatAlign.CENTER);
			...
			///////////
//			title.txtTitle.htmlText = '<font size="18">' + UIConstData.ItemDic[data.type].Name + '</font>';
		...
		private function setContent1():void
		{
			content1 = new Sprite();
			var typeMul:uint = data.type / 1000;
			if(typeMul == 351) {
				moneyItem = new MoneyItem(50);
				moneyItem.update( data.amountMoney + "\\ab" );
				...
				tf.textColor = 0x0098ff;
				tf.text = GameCommonData.wordDic[ "mod_too_med_ui_SetCon" ]+"：";//"元宝面额："
				var _w:int;
				if(int(data.amountMoney/10) >= 100)
				{
					_w = 55;
				}
				else if(int(data.amountMoney/10) >= 10)
				{
					_w = 50;
				}
				else if(int(data.amountMoney/10) >= 1)
				{
					_w = 45;
				}
				else if(int(data.amountMoney/10) == 0)
				{
					_w = 40;
				}
				moneyItem.x = tf.x + _w;
				moneyItem.y = tf.y;
				content1.addChild(tf);
				content1.addChild(moneyItem);
				
			}
			var frame:MovieClip = GameCommonData.GameInstance.Content.Load(GameConfigData.UILibrary).GetClassByMovieClip("mcFrameOne");
			content1.addChildAt(frame, 0);
			frame.height = content1.height;
			content1.mouseEnabled = false;
			content1.mouseChildren = false;
			toolTip.addChild(content1);
		}

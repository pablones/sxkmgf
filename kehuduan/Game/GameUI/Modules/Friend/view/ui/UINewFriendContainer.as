			...
			var group:int = 0;
				for each(var data:Array in arr){
					for(var i:uint=0;i<data.length;i++){
						if(this.roleList[i + group * 10] == null)
						{
							var val:PlayerInfoStruct=data[i].roleInfo as PlayerInfoStruct;	
		...
		protected function sortOn(arr:Array):void{
			for(var i:uint=0;i<arr.length;i++){
				for(var j:uint=0;j<arr.length-i-1;j++){
					var val1:PlayerInfoStruct=arr[j] as PlayerInfoStruct;
					var val2:PlayerInfoStruct=arr[j+1] as PlayerInfoStruct;
...
//						{cellText:GameCommonData.wordDic[ "mod_fri_view_med_uie_cre" ],data:{hasChild:false,type:GameCommonData.wordDic[ "mod_fri_view_med_uie_cre" ]}},//"邀请入帮"		"邀请入帮"
		...
		public function showMenu(playerInfo:PlayerInfoStruct):void{
			...
			dataList.push({cellText:GameCommonData.wordDic[ "mod_fri_view_med_friendM_han_4" ],data:{type:GameCommonData.wordDic[ "mod_fri_view_med_friendM_han_4" ]}});//"分组"		"分组"
